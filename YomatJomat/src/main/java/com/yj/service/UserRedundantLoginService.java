...
@Log4j
@AllArgsConstructor
public class UserRedundantLoginService implements HttpSessionBindingListener,Serializable {
	 ...
	 */
	public boolean isUsing(String email) {
		return loginUsers.containsValue(email);
	}
	 ...
	 * @param loginId 사용자 아이디
	 */
	public void setSession(HttpSession session, String email) {
		// 이순간에 Session Binding이벤트가 일어나는 시점
		// name값으로 loginId, value값으로 자기자신(HttpSessionBindingListener를 구현하는 Object)
		session.setAttribute(email, this);// login에 자기자신을 집어넣는다.
	}
 ...
 * 로그인을 하면 접속을 끊을지를 물어보는 기능도 보셨을 겁니다. 이를 웹에 서 구현하여 보았습니다. 
